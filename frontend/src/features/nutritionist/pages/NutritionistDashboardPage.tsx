			...
			<Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr" }, gap: 2 }}>
				...
				<SectionCard title="Solicitações de acompanhamento">
					{requestsLoading ? (
						...
						requests.map((req) => (
							<TrackingRequestRow key={req.id} request={req} onRespond={handleRespond} />
						))
					)}
				</SectionCard>
						...
						patients.slice(0, 5).map((p) => (
							...
							>
								...
								<Box>
									...
									<Typography
										sx={{ fontSize: "0.72rem", color: theme.palette.typography.secondaryCardText }}
									>
										{p.email}
									</Typography>
								</Box>
...
function StatCard({
	...
	return (
		...
		>
			...
			<Typography
				sx={{ fontSize: "1.8rem", fontWeight: 700, color: theme.palette.typography.mainText }}
			>
				{value}
			</Typography>
			<Typography
				sx={{ fontSize: "0.78rem", color: theme.palette.typography.secondaryText, mt: 0.25 }}
			>
				{label}
			</Typography>
		</Box>
