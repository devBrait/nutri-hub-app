  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        enqueueSnackbar(res.message ?? "Não foi possível atualizar o perfil.", { variant: "error" });
      }
    } catch (err) {
...
function FieldRow({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <Box sx={{ mb: 1.75 }}>
      <Typography sx={{ fontSize: "0.78rem", fontWeight: 600, color: theme.palette.typography.secondaryText, mb: 0.6 }}>
        {label}
      </Typography>
      {children}
    </Box>
