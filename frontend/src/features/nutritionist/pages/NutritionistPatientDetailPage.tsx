...
import { getPatientDailySummaryByNutritionist, getPatientProfileByNutritionist, type MealSummary } from "../../../lib/api/patient.service";
...
export default function NutritionistPatientDetailPage() {
  ...
  return (
    <Box sx={{ pb: { xs: 2, md: 0 } }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "340px 1fr" }, gap: 2, alignItems: "start" }}>
        {/* Profile card */}
        <SectionCard title="Perfil do paciente">
          {profileLoading ? (
            [0, 1, 2, 3, 4].map((i) => (
              <Skeleton key={i} variant="rounded" height={28} sx={{ mb: 1, bgcolor: theme.palette.neutral.altTempBackground }} />
            ))
          ) : profile ? (
            <>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2.5 }}>
                <Box
                  sx={{
                    ...
                    fontSize: "1.3rem", fontWeight: 700, color: theme.palette.brand.main,
                  }}
                >
                  {profile.name.slice(0, 1).toUpperCase()}
                </Box>
                <Box>
                  <Typography sx={{ fontSize: "1rem", fontWeight: 700, color: theme.palette.typography.mainText }}>
                    {profile.name}
                  </Typography>
              ...
              <ProfileRow label="Objetivo" value={profile.objective ? (OBJECTIVE_LABELS[profile.objective] ?? profile.objective) : "—"} />
              <ProfileRow label="Nível de atividade" value={profile.activityLevel ? (ACTIVITY_LABELS[profile.activityLevel] ?? profile.activityLevel) : "—"} />
              <ProfileRow label="Meta calórica diária" value={profile.dailyCalorieGoal ? `${profile.dailyCalorieGoal} kcal` : "—"} />
              <ProfileRow label="Peso atual" value={profile.currentWeightKg ? `${profile.currentWeightKg} kg` : "—"} />
              <ProfileRow label="Peso alvo" value={profile.targetWeightKg ? `${profile.targetWeightKg} kg` : "—"} />
              <ProfileRow label="Altura" value={profile.heightCm ? `${profile.heightCm} cm` : "—"} />
              <ProfileRow label="Idade" value={profile.ageYears ? `${profile.ageYears} anos` : "—"} />
            </>
          ) : (
        ...
        <Box>
          ...
          <Box
            sx={{
              ...
              mb: 2, bgcolor: theme.palette.neutral.card,
              border: `1px solid ${theme.palette.divider}`,
              borderRadius: "16px", p: "12px 16px",
            }}
          >
            <IconButton size="small" onClick={() => shiftDate(-1)}>
              <ChevronLeftIcon />
            </IconButton>
            <Typography sx={{ fontSize: "0.88rem", fontWeight: 600, color: theme.palette.typography.mainText }}>
              {isToday
                ? "Hoje"
          ...
          {summaryLoading ? (
              ...
              <SectionCard title="Refeições do dia">
                {summary.meals.length === 0 ? (
                  ...
                  summary.meals.map((meal) => (
                    ...
                    >
                      <Box>
                        <Typography sx={{ fontSize: "0.85rem", fontWeight: 600, color: theme.palette.typography.mainText }}>
                          {MEAL_LABELS[meal.mealType] ?? meal.mealType}
                        </Typography>
                        <Typography sx={{ fontSize: "0.72rem", color: theme.palette.typography.secondaryCardText }}>
                          C: {Math.round(meal.carbsG)}g · P: {Math.round(meal.proteinG)}g · G: {Math.round(meal.fatG)}g
                        </Typography>
                      </Box>
                      <Typography sx={{ fontSize: "0.88rem", fontWeight: 700, color: theme.palette.brand.main }}>
            ...
            <Typography sx={{ fontSize: "0.82rem", color: theme.palette.typography.secondaryCardText, py: 4, textAlign: "center" }}>
...
function ProfileRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.25 }}>
      <Typography sx={{ fontSize: "0.78rem", color: theme.palette.typography.secondaryCardText }}>{label}</Typography>
      <Typography sx={{ fontSize: "0.82rem", fontWeight: 600, color: theme.palette.typography.mainText }}>{value}</Typography>
    </Box>
  );
}

function MacroBox({ label, value, goal, color }: { label: string; value: string; goal?: string; color: string }) {
  const theme = useTheme();
  return (
    <Box sx={{ bgcolor: alpha(color, 0.08), borderRadius: "12px", p: 1.25, textAlign: "center" }}>
      <Typography sx={{ fontSize: "0.68rem", color: theme.palette.typography.secondaryCardText, mb: 0.25 }}>
        {label}
      </Typography>
      <Typography sx={{ fontSize: "0.95rem", fontWeight: 700, color }}>
        {value}
        {goal && <Box component="span" sx={{ fontSize: "0.7rem", fontWeight: 400, color: theme.palette.typography.secondaryCardText }}>{goal}</Box>}
      </Typography>
    </Box>
  );
}
