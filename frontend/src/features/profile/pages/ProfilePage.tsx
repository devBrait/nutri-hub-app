					...
					{profile.weightHistory.map((entry) => (
						...
						>
							...
							<Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
								...
								>
									{entry.date.slice(8, 10)}/{entry.date.slice(5, 7)}/{entry.date.slice(0, 4)}
								</Typography>
...
function HeaderStat({ value, label }: { value: string; label: string }) {
	return (
		...
		>
			<Typography sx={{ fontSize: "1.35rem", fontWeight: 700, color: "#fff" }}>{value}</Typography>
			<Typography sx={{ fontSize: "0.7rem", color: alpha("#fff", 0.65), mt: 0.25 }}>
				{label}
			</Typography>
		</Box>
	);
