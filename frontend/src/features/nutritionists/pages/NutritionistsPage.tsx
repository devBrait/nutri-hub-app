...
import { type LinkedNutritionist, useNutritionists } from "../../../hooks/useNutritionists";
...
export default function NutritionistsPage() {
	...
	const { nutritionists, linkedNutritionist, pendingRequestIds, loading, refetch } =
		useNutritionists();
			...
			{loading ? (
				<>
					<Skeleton
						variant="rounded"
						height={110}
						sx={{ borderRadius: "16px", mb: 3, bgcolor: theme.palette.neutral.altTempBackground }}
					/>
					<Box sx={gridSx}>
						{[0, 1, 2].map((i) => (
							<Skeleton
		...
		>
			...
			<Box sx={{ flex: 1, minWidth: 0 }}>
				<Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.25 }}>
					<Typography
						sx={{ fontSize: "0.95rem", fontWeight: 700, color: theme.palette.typography.mainText }}
					>
						{nutritionist.name}
					</Typography>
					<Box
				...
				<Typography sx={{ fontSize: "0.75rem", color: theme.palette.typography.secondaryCardText }}>
					{nutritionist.email} · vinculado em{" "}
					{new Date(nutritionist.linkedAt).toLocaleDateString("pt-BR")}
				</Typography>
			</Box>
			...
			<Button
				...
				startIcon={
					unlinking ? (
						...
						<LinkOffIcon sx={{ fontSize: "1rem !important" }} />
					)
				}
				sx={{
...
function NutritionistCard({
	...
	const handleRequest = async () => {
		...
		try {
			...
			if (res.success) {
				enqueueSnackbar(
					`Vinculado a ${res.output?.nutritionistName ?? "nutricionista"} com sucesso!`,
					{ variant: "success" },
				);
				onLinked();
			} else {
				enqueueSnackbar(res.message ?? "Não foi possível solicitar acompanhamento.", {
					variant: "error",
				});
			}
		} catch (err) {
		...
		>
			...
			<Button
				...
				sx={{
					...
					color: isConnected ? theme.palette.brand.main : isPending ? "#f59e0b" : "#fff",
					border: isConnected
